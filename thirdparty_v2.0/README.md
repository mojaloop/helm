...
1. `tpp-api-svc-2` — the new Third Party API Service
...
## Why does `tpp-api-svc-2` depend on `consent-oracle` and `auth-svc`?
...
- [chart-tpp-api-svc-2](./chart-tpp-api-svc-2) — the new Third Party API Service (`tpp-api-svc`). Copied from `chart-tp-api-svc`, then changed to match the new service — new image, new port, new config.
